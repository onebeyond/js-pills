...
const generateRandomArray = (length, diffValues) => [... new Array(length)].map(() => randomIntegers(diffValues));
const mean = array => array.reduce((acc, value) => acc + value, 0) / array.length;
const normalizedResults = resultObject => Object.keys(resultObject).reduce((acc, key) => { acc[key] = mean(resultObject[key]); return acc; }, {});
...
const dedupe = array => dedupeMethod=> {
  ...
  console.log(`Finished (deduped array length: ${dedupedArray.length}) method ${dedupeMethod.toString()}: ${duration} ms`)
...
[... new Array(ITERATIONS)].forEach(() => {
  ...
  console.log(`random array length: ${array.length} - random numbers: ${RANDOM_RANGE}`);
